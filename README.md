...
应届运维工程师的真实项目留档：云原生 GitOps 交付链路（K8s + Harbor + Jenkins + ArgoCD）、企业级 WAF 安全防护（雷池 + Nginx 高可用）；另附数据库自学期间的 MySQL 主从复制排障记录。
...
## 学习记录：MySQL 主从复制排障实录（IO / SQL 两类线程故障）

数据库自学练手记录，非实际项目。在本地两台虚拟机搭的主从环境（MySQL 8.1.0 主 / 8.0.46 从）上，排了复制中断的两类典型故障：
